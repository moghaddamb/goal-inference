...
class Experiment(object):
    ...
    def __call__(self, trial=None, design={}, writer=None):
        ...
        for (i, d) in enumerate(design):
            ...
            if trial.result["target"] == trial.result["inferred_target"]:
                right += 1
            print("YOUR CURRENT ACCURACY: " + 4str((right/(i+1))))
...
def draw_accuracy(frame, accuracy, x=500, y=300, size=0.41, color=(255,255,255), thickness=1, line = cv2.LINE_AA):
...
class Display(object):
    ...
    def __init__(self,
        ...
        cv2.setWindowProperty(self._name, cv2.WND_PROP_AUTOSIZE, cv2.WINDOW_AUTOSIZE)
...
def main():
    ...
    variables = dict([("actor", actor_names),
                      ...
                      ("percent", [1.0])])
    design_values = design(variables=variables, num_repetition=4)
